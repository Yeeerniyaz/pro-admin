 ...
 * @description Экран управления прайс-листом и системными настройками (PROADMIN Mobile v10.0.0).
 * Позволяет администратору динамически менять цены на услуги.
 * Реализовано массовое сохранение (Bulk Update) и оптимизированный ввод.
 *
...
export default function SettingsScreen() {
  ...
  const handlePriceChange = (categoryIndex, itemIndex, newPriceStr) => {
    // Копируем стейт для иммутабельности
    const updatedPricelist = [...pricelist];
    // Обновляем конкретное значение
            ...
            {pricelist.length === 0 ? (
              ...
              pricelist.map((section, catIdx) => (
                <View key={`cat-${catIdx}`} style={styles.categoryBlock}>
                  {/* Заголовок категории (например: "Черновые работы") */}
                  <View style={styles.categoryHeader}>
                    <Text style={styles.categoryTitle}>{section.category}</Text>
                  </View>

                  {/* Карточка с инпутами для этой категории */}
                  <PeCard style={styles.itemsCard}>
                      ...
                      <View key={`item-${item.key}`} style={styles.itemRow}>
                        ...
                        <View style={styles.inputWrapper}>
                          <PeInput
                            ...
                            style={{ marginBottom: 0 }} // Переопределяем дефолтный отступ PeInput
                            placeholder="0"
                          />
                        </View>
                      </View>
...
const styles = StyleSheet.create({
  ...
  inputWrapper: {
    width: 100, // Фиксированная ширина для полей ввода цен
  },
