 ...
 * @description Центр уведомлений и рассылок (PROADMIN Mobile v10.0.0).
...
import { Radio, Send, Users, ShieldAlert, Image as ImageIcon } from 'lucide-react-native';
...
const TARGET_OPTIONS = [
  { id: 'all', label: 'Всем (Общая)', icon: <Radio color={COLORS.textMuted} size={16} /> },
  { id: 'user', label: 'Только Клиентам', icon: <Users color={COLORS.textMuted} size={16} /> },
  { id: 'manager', label: 'Персоналу (Мастера)', icon: <ShieldAlert color={COLORS.textMuted} size={16} /> },
];
...
export default function BroadcastScreen() {
  ...
  const executeBroadcast = async () => {
    ...
    try {
      const res = await API.sendBroadcast(message, imageUrl || null, targetRole);
      
      // Бэкенд возвращает success и message с количеством получателей
      Alert.alert('Рассылка запущена', res.message || 'Сообщения отправляются в фоновом режиме.');
      
      // Очищаем форму после успешной отправки
      setMessage('');
      setImageUrl('');
    } catch (err) {
      Alert.alert('Ошибка рассылки', err.message || 'Проверьте соединение с сервером');
    } finally {
      setLoading(false);
    }
  };
        ...
        <View style={{ flex: 1 }}>
          ...
          <View style={styles.header}>
            <View style={GLOBAL_STYLES.rowCenter}>
              ...
              <View>
                ...
                <Text style={GLOBAL_STYLES.textMuted}>Управление Telegram-рассылкой</Text>
              </View>
            </View>
          </View>
          ...
          >
            <PeCard style={{ padding: SIZES.large }}>
                ...
                {TARGET_OPTIONS.map((opt) => {
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      style={[styles.targetBtn, isActive && styles.targetBtnActive]}
                    >
                      {/* Клонируем иконку, чтобы покрасить ее, если она активна */}
                      {React.cloneElement(opt.icon, { color: isActive ? COLORS.primary : COLORS.textMuted })}
              ...
              <Text style={[styles.sectionTitle, { marginTop: SIZES.medium }]}>2. Изображение (URL, опционально)</Text>
              <PeInput
                value={imageUrl}
              ...
              {/* 3. Текст рассылки */}
              <Text style={styles.sectionTitle}>3. Текст сообщения (поддерживает HTML)</Text>
              ...
              <View style={styles.infoBox}>
                <Text style={GLOBAL_STYLES.textSmall}>
                  Рассылка осуществляется через официального бота ProElectric. Доставка занимает время в зависимости от размера базы.
                </Text>
              </View>
...
const styles = StyleSheet.create({
  ...
  // Текстовая область
  textAreaContainer: {
    minHeight: 150, // Делаем поле визуально большим
  },
  textArea: {
    minHeight: 150,
    paddingTop: SIZES.small, // Исправляет прилипание текста к верхней рамке
  },
