 ...
 * @description Главный экран аналитики (PROADMIN Mobile v14.0.0 Enterprise).
 * 🔥 ИСПРАВЛЕНО (v14.0.0): Железобетонная синхронизация путей с бэкендом (/api/analytics/mobile/dashboard/stats).
 * 🔥 ИСПРАВЛЕНО (v14.0.0): Защита от падений при ответе сервера HTML-страницей (ошибка 404/502).
 *
 * @module DashboardScreen
 * @version 14.0.0 (Sync & Crash-Free Edition)
 */
...
export default function DashboardScreen() {
  ...
  const fetchDashboardData = async (isRefresh = false, selectedPeriod = period) => {
    try {
      ...
      // 🔥 ИСПРАВЛЕНО: Точные URL и безопасный парсинг JSON
      let statsPromise;
      if (typeof API.getDashboardStats === 'function') {
        statsPromise = API.getDashboardStats(startDate, endDate);
      } else {
        statsPromise = fetch(`https://erp.yeee.kz/api/analytics/mobile/dashboard/stats?startDate=${startDate}&endDate=${endDate}`, { headers })
          .then(async res => {
            const text = await res.text();
            try { return JSON.parse(text); }
            catch (e) { throw new Error("Неверный ответ сервера (Дашборд)"); }
          });
      }
      ...
      if (typeof API.getDeepAnalytics === 'function') {
        ...
        deepPromise = fetch(`https://erp.yeee.kz/api/analytics/deep?startDate=${startDate}&endDate=${endDate}`, { headers })
          .then(async res => {
            const text = await res.text();
            try { return JSON.parse(text); }
            catch (e) { throw new Error("Неверный ответ сервера (Аналитика)"); }
          });
      }
  ...
  return (
    <View style={GLOBAL_STYLES.safeArea}>
      ...
      <View style={styles.filterContainer}>
        <TouchableOpacity style={[styles.filterBtn, period === 'all' && styles.filterBtnActive]} onPress={() => setPeriod('all')} activeOpacity={0.8}>
          <Text style={[styles.filterText, period === 'all' && styles.filterTextActive]}>За всё время</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[styles.filterBtn, period === 'month' && styles.filterBtnActive]} onPress={() => setPeriod('month')} activeOpacity={0.8}>
      ...
      >
        ...
        <View style={styles.kpiGrid}>
          <PeCard elevated={false} style={[styles.kpiCard, { borderColor: COLORS.success }]}>
            ...
            <Text style={[styles.kpiValue, { color: COLORS.success }]} numberOfLines={1} adjustsFontSizeToFit>{formatKZT(overview.totalNetProfit)}</Text>
          </PeCard>
          ...
          <PeCard elevated={false} style={[styles.kpiCard, { borderColor: COLORS.primary }]}>
            ...
            <Text style={styles.kpiValue} numberOfLines={1} adjustsFontSizeToFit>{formatKZT(overview.totalRevenue)}</Text>
          </PeCard>
          ...
          {isAdmin && (
            <PeCard elevated={false} style={[styles.kpiCard, { borderColor: COLORS.danger }]}>
              ...
              <Text style={[styles.kpiValue, { color: COLORS.danger }]} numberOfLines={1} adjustsFontSizeToFit>{formatKZT(economics.totalBrigadeDebts || 0)}</Text>
            </PeCard>
          )}
          ...
          <PeCard elevated={false} style={[styles.kpiCard, { borderColor: COLORS.warning }]}>
            ...
            <Text style={styles.kpiValue} numberOfLines={1} adjustsFontSizeToFit>{formatKZT(economics.averageCheck || 0)}</Text>
          </PeCard>
              ...
              <View key={status}>
                <View style={styles.funnelRow}>
                  ...
                  <View style={{ alignItems: "flex-end" }}>
                    ...
                    <Text style={[GLOBAL_STYLES.textSmall, { color: COLORS.textMuted }]}>{formatKZT(stat.sum)}</Text>
                  </View>
                </View>
...
const styles = StyleSheet.create({
  header: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingHorizontal: SIZES.large, paddingTop: SIZES.large, paddingBottom: SIZES.medium, backgroundColor: COLORS.background, borderBottomWidth: 1, borderBottomColor: COLORS.border },
  iconWrapper: { width: 44, height: 44, borderRadius: SIZES.radiusMd, backgroundColor: "rgba(255, 107, 0, 0.1)", justifyContent: "center", alignItems: "center", marginRight: SIZES.medium },
  logoutButton: { padding: 10, backgroundColor: "rgba(239, 68, 68, 0.1)", borderRadius: SIZES.radiusSm, borderWidth: 1, borderColor: "rgba(239, 68, 68, 0.3)" },
  filterContainer: { flexDirection: 'row', paddingHorizontal: SIZES.large, paddingVertical: SIZES.medium, gap: SIZES.small, backgroundColor: COLORS.background },
  filterBtn: { paddingVertical: 8, paddingHorizontal: 16, borderRadius: 20, borderWidth: 1, borderColor: COLORS.border, backgroundColor: COLORS.surfaceElevated },
  filterBtnActive: { borderColor: COLORS.primary, backgroundColor: "rgba(255, 107, 0, 0.15)" },
  filterText: { color: COLORS.textMuted, fontSize: SIZES.fontSmall, fontWeight: '600' },
  filterTextActive: { color: COLORS.primary },
  scrollView: { flex: 1 },
  scrollContent: { padding: SIZES.large },
  kpiGrid: { flexDirection: "row", flexWrap: "wrap", justifyContent: "space-between" },
  kpiCard: { width: "48%", padding: SIZES.medium, marginBottom: SIZES.medium },
  kpiIconWrapper: { width: 32, height: 32, borderRadius: SIZES.radiusSm, justifyContent: "center", alignItems: "center", marginBottom: SIZES.small },
  kpiLabel: { fontSize: 11, color: COLORS.textMuted, textTransform: "uppercase", marginBottom: 4, letterSpacing: 0.5 },
  kpiValue: { fontSize: SIZES.fontMedium, fontWeight: "700", color: COLORS.textMain },
  funnelCard: { paddingHorizontal: 0, paddingVertical: SIZES.small },
  funnelRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingVertical: SIZES.small, paddingHorizontal: SIZES.medium },
  divider: { height: 1, backgroundColor: COLORS.border, width: "100%" },
  errorBox: { backgroundColor: "rgba(239, 68, 68, 0.1)", borderWidth: 1, borderColor: "rgba(239, 68, 68, 0.3)", padding: SIZES.small, borderRadius: SIZES.radiusSm, marginBottom: SIZES.medium },
  errorText: { color: COLORS.danger, fontSize: SIZES.fontSmall, textAlign: "center" },
});